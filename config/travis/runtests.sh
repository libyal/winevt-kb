...
then
	...
	if test -n "${TOXENV}";
	then
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd winevt-kb && tox -e ${TOXENV}";

	elif test ${TRAVIS_PYTHON_VERSION} = "2.7";
	then
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd winevt-kb && python2 run_tests.py";
	else
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd winevt-kb && python3 run_tests.py";
	fi
