...
then
	...
	docker exec "${CONTAINER_NAME}" sh -c "git clone https://github.com/libyal/winevt-kb.git";

	if test ${TRAVIS_PYTHON_VERSION} = "2.7";
	then
		docker exec "${CONTAINER_NAME}" sh -c "cd winevt-kb && python2 run_tests.py";
	else
		docker exec "${CONTAINER_NAME}" sh -c "cd winevt-kb && python3 run_tests.py";
	fi
