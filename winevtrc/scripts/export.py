...
class DocumentationFilesOutputWriter(exporter.ExporterOutputWriter):
    ...
    def Open(self):
        ...
        index_rst_file_path = os.path.join(self._message_files_directory, "index.rst")
        self._message_files_index_writer = documentation.MessageFilesIndexRstWriter(
            index_rst_file_path
        )
    ...
    def WriteEventLogProvider(self, export_event_log_provider):
        ...
        output_filename = f"Provider-{name:s}.md".replace(" ", "-").replace("/", "-")
    ...
    def WriteMessageFile(self, message_file):
        ...
        output_filename = f"MessageFile-{name:s}.md".replace(" ", "-").replace("/", "-")
...
class StdoutOutputWriter(exporter.ExporterOutputWriter):
    ...
    def WriteEventLogProvider(self, export_event_log_provider):
        ...
        Args:
          export_event_log_provider (ExportEventLogProvider): Event Log provider.
        """
        for event_log_provider, _ in export_event_log_provider.providers_with_versions:
