...
class EventLogProviderMarkdownWriter(DocumentationFileWriter):
    ...
    def WriteEventLogProvider(self, event_log_provider, windows_versions):
        ...
        for index, path in enumerate(
            sorted((event_log_provider.category_message_files))
        ):
            if index == 0:
                lines.extend(
                    [
        ...
        for index, path in enumerate(sorted((event_log_provider.event_message_files))):
            if index == 0:
                lines.extend(
                    [
                        "    <tr>",
        ...
        for index, path in enumerate(
            sorted((event_log_provider.parameter_message_files))
        ):
            if index == 0:
                lines.extend(
                    [
...
class MessageFilesIndexRstWriter(DocumentationFileWriter):
    ...
    def WriteMessageFile(self, name):
        ...
        """
        output_filename = f"MessageFile-{name:s}".replace(" ", "-").replace("/", "-")
        self._file_object.write(f"   {name:s} <{output_filename:s}>\n")
