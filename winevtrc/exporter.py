...
class MessageFileAttributeContainerStore(sqlite_store.SQLiteAttributeContainerStore):
...
class ExporterOutputWriter:
    ...
    def _DiffMessageString(
        ...
        matcher = difflib.SequenceMatcher(None, message_string, other_message_string)
                    ...
                    for line_opcode, a0, a1, b0, b1 in line_matcher.get_opcodes():
                        if line_opcode == "equal":
                            ...
                            second_output.append(
                                f"{color_green:s}{text:s}{color_end:s}"
                            )
    ...
    def GetMessageTables(self, message_file):
        ...
        for extracted_message_table in message_file.GetAttributeContainers(
            ...
            extracted_message_file = message_file.GetAttributeContainerByIdentifier(
        ...
        for file_version_tuple, extracted_message_table in sorted(
            ...
            for extracted_message_string in message_file.GetAttributeContainers(
        ...
        for other_file_version, other_message_table in message_tables[1:]:
            if message_table.message_strings == other_message_table.message_strings:
                ...
                message_identifiers.update(
                    set(other_message_table.message_strings.keys())
                )
                for message_identifier in message_identifiers:
                    ...
                    if not message_string:
                        ...
                        logging.warning(
                            f"Message string: 0x{message_identifier:08x} was removed "
                            f"in LCID: 0x{language_identifier:08x} and version: "
                            f"{other_file_version:s}."
                        )
                        ...
                        self._DiffMessageString(
                            file_version,
                            message_table.language_identifier,
                            message_identifier,
                            [message_string],
                            [other_message_string],
                        )
...
class Exporter:
    ...
    def _CompareEventLogProviders(self, event_log_provider, other_event_log_provider):
        ...
        for event_log_provider in database_reader.GetAttributeContainers(
            ...
            if not export_event_log_provider:
                ...
                for (
                    ...
                    windows_versions,
                ) in export_event_log_provider.providers_with_versions:
    ...
    def _ExportMessageResourceFiles(self, source_path, database_reader, output_writer):
