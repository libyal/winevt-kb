...
class ResourcesSQLite3DatabaseWriter:
        ...
        if not has_table:
            ...
            if number_of_values == 1:
                ...
                if message_string != stored_message_string:
                    logging.warning(
                        f"Message string mismatch for LCID: "
                        f"0x{language_identifier:04x}, message identifier: "
                        f"{message_identifier:s}.\nFound: {message_string:s}\n"
                        f"Stored: {stored_message_string:s}\n"
                    )
    ...
    def WriteEventLogProvider(self, event_log_provider):
        ...
        if insert_values:
            values = [event_log_provider.log_source, event_log_provider.identifier]
            self._database_file.InsertValues(table_name, column_names, values)
