...
class RequirementsWriter(object):
  ...
  def Write(self):
    ...
    for dependency in winevtrc.dependencies.GetInstallRequires():
...
class SetupCfgWriter(object):
  ...
  def Write(self):
    ...
    dependencies = winevtrc.dependencies.GetRPMRequires()
...
class TravisBeforeInstallScript(object):
  ...
  _FILE_FOOTER = [
      ...
      (u'\tPYTHONPATH=../l2tdevtools ../l2tdevtools/tools/update.py '
       u'--download-directory=dependencies --preset=winevt-kb;'),
      u'',
  ...
  def Write(self):
    ...
    dependencies = winevtrc.dependencies.GetDPKGDepends(exclude_version=True)
    dependencies = u' '.join(dependencies)
    file_content.append(u'PYTHON2_DEPENDENCIES="{0:s}";'.format(dependencies))

    file_content.append(u'')

    # TODO: determine test dependencies from winevtrc.dependencies.
